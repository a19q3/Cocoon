...
| P1.2e installed capsule entrypoint FD launch spike | PARTIAL | Redox/QEMU opens the installed capsule entrypoint before restriction, enters a manifest-derived restricted namespace, fexecs the entrypoint FD, and proves declared resource access plus denied path/scheme rejection. | `cocoon probe-capsule-fd-launch` writes a `capsule_fd_launch_probe` receipt with `authority_mode = redox-enforced-capsule-entrypoint`; QEMU smoke includes `PASS probe Redox FD-only installed capsule entrypoint inside redox` and `PASS audit Redox FD-only launch probe receipts inside redox`. | Run this boundary across multiple service profiles before promoting `cocoon run` to `redox-enforced`. |
