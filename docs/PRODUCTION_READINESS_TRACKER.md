...
| Redox authority enforcement | PARTIAL | `cocoon run` constructs a Redox namespace, passes preopened handles, and proves denied scheme/path access fails. | P1.2a/P1.2b `probe-authority` validates Redox null namespace behavior through a restricted child process and audited receipt; P1.2c `probe-fd-exec` classifies the path-exec blocker; P1.2d `probe-fd-launch` probes controlled fixture launch from an inherited executable FD without changing production `run`. Current `run` still fails closed unless `--allow-unenforced-authority` and receipts record `smoke-unenforced`. | Implement actual arbitrary installed-service FD-only launch and remove smoke override from production path. |
...
| P1.2d controlled FD-only launch spike | PARTIAL | Redox/QEMU either launches a controlled fixture from an inherited executable FD under the null namespace or records a precise `redox-fd-launch-blocked` upstream API gap. | `cocoon probe-fd-launch` writes an `fd_launch_probe` receipt and QEMU smoke accepts PASS or BLOCKED evidence. | Use the result to ask Redox/Ibuki about the stable `fexecve`/FD launch contract, then graduate to arbitrary capsule service launch. |
...
| Machine-readable CLI contract | DONE | Core commands support stable JSON output and documented exit codes for automation. | `plan`, `install`, `run`, `probe-authority`, `probe-fd-exec`, `probe-fd-launch`, `status`, `logs`, `check-install`, `rollback`, `recover`, and `audit` support `--json`; CLI golden parses JSON for `plan`, `run`, `status`, `logs`, and `audit`; `docs/CLI_CONTRACT.md` documents output and exit-code semantics. | Keep JSON fixtures stable as commands evolve. |
...
Latest local evidence on 2026-05-17:
